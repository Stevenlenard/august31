...
class _ResidentComplaintsScreenState extends State<ResidentComplaintsScreen> with TickerProviderStateMixin {
  ...
  void _applyFilters() {
    ...
    // 4. Filter by Date (only if active)
  ...
  void _handleDelete(int index) async {
    if (index < 0 || index >= _complaints.length) return;
    final complaint = _complaints[index];
    final String cid = (complaint['complaint_id'] ?? complaint['id'] ?? '0').toString();
    final int id = int.tryParse(cid) ?? 0;
    if (id == 0) return;
      ...
      try {
        ...
        if (response.data['success'] == true) {
          ...
          _listKey.currentState?.removeItem(
            index,
            (context, animation) => _buildDismissibleItem(removedItem, index, animation, isRemoving: true),
          ...
          _showSnackBar("Complaint permanently deleted.");
        } else {
          _showSnackBar(response.data['message'] ?? "Deletion failed.", isError: true);
        }
      } catch (e) {
        _showSnackBar("Network error during deletion.", isError: true);
      }
    }
  }
  ...
  void _handleClearAll() async {
    // 1. Identify which IDs to clear based on mode
    final List<int> idsToClear = _isSelectionMode 
        ? _selectedIds.map((id) => int.tryParse(id) ?? 0).where((id) => id != 0).toList()
        : _complaints.map((c) => int.tryParse((c['complaint_id'] ?? c['id'] ?? '0').toString()) ?? 0).where((id) => id != 0).toList();

    if (idsToClear.isEmpty) {
      _showSnackBar("Please select at least one complaint to delete.", isError: true);
      return;
    }

    bool confirmed = await _showConfirmDialog(
      title: _isSelectionMode ? "Delete Selected?" : "Clear All History?",
      message: _isSelectionMode 
          ? "Are you sure you want to delete ${idsToClear.length} selected records?" 
          : "Are you sure you want to delete ALL your records? This cannot be undone.",
    );

    if (confirmed) {
      try {
        final response = await _apiService.bulkDeleteComplaints(idsToClear);
        
        if (response.data['success'] == true) {
          // Sequential Animation and permanent cleanup
          final List<String> stringIdsToRemove = idsToClear.map((e) => e.toString()).toList();
          
          for (String sid in stringIdsToRemove) {
            // Important: We must find the index EACH time because it shifts after removal
            int idx = _complaints.indexWhere((c) => (c['complaint_id'] ?? c['id']).toString() == sid);
            if (idx != -1) {
              final removedItem = _complaints.removeAt(idx);
              _allComplaints.removeWhere((c) => (c['complaint_id'] ?? c['id']).toString() == sid);
              await _deleteFromFirebase(int.tryParse(sid) ?? 0);
              
              _listKey.currentState?.removeItem(
                idx,
                (context, animation) => _buildDismissibleItem(removedItem, idx, animation, isRemoving: true),
          ...
          if (widget.onDataChanged != null) widget.onDataChanged!();
          _showSnackBar("Records successfully deleted.");
        } else {
          _showSnackBar(response.data['message'] ?? "Deletion failed on server.", isError: true);
        }
      } catch (e) {
        _showSnackBar("Error during deletion: $e", isError: true);
      }
    }
  }
                    ...
                    children: [
                      ...
                      Expanded(
                        child: HoverActionButton(
                            text: "CONFIRM",
                            isDestructive: title.contains("Delete") || title.contains("Clear"),
  ...
  Widget _buildDesktopLayout() {
    ...
    return Scaffold(
      backgroundColor: Colors.white,
      floatingActionButton: _isSelectionMode ? null : _buildFloatingAddButton(),
      body: Stack(
        children: [
          Listener(
                                        ...
                                        child: Row(
                                          children: [
                                            Column(
                                              crossAxisAlignment: CrossAxisAlignment.start,
                                              children: [
                                                Text(_isSelectionMode ? "Select Records" : "Submission History",
                                                ...
                                                Text(_isSelectionMode 
                                                    ...
                                                    style: TextStyle(color: _isSelectionMode ? const Color(0xFF00897B) : Colors.grey, fontSize: 14, fontWeight: FontWeight.w500)),
                                              ],
                                            ),
                                            const Spacer(),
                                            if (_isSelectionMode) ...[
                                              _HoverZoomLink(
                                                onTap: () {
                                                  setState(() {
                                                    if (_selectedIds.length == _complaints.length) {
                                                      _selectedIds.clear();
                                                    } else {
                                                      for (var c in _complaints) {
                                                        _selectedIds.add((c['complaint_id'] ?? c['id']).toString());
                                                      }
                                                    }
                                                  });
                                                },
                                                child: const Padding(
                                                  ...
                                                  child: Text("SELECT ALL", style: TextStyle(color: Colors.green, fontWeight: FontWeight.w900, fontSize: 13)),
                                                ),
                                              ),
                                              _HoverZoomLink(
                                                ...
                                                child: const Padding(
                                                  padding: EdgeInsets.symmetric(horizontal: 12),
                                                  child: Text("CANCEL", style: TextStyle(color: Colors.grey, fontWeight: FontWeight.w900, fontSize: 13)),
                                                ),
                                              ),
                                              if (_selectedIds.isNotEmpty)
                                                _HoverZoomLink(
                                                  ...
                                                  child: const Padding(
                                                    padding: EdgeInsets.symmetric(horizontal: 12),
                                                    child: Text("DELETE", style: TextStyle(color: Colors.red, fontWeight: FontWeight.w900, fontSize: 13)),
                                                  ),
                                                ),
                                            ],
                                            if (!_isSelectionMode)
                                    ...
                                    Padding(
                                      ...
                                      child: Align(
                                        ...
                                        child: Column(
                                          ...
                                          children: [
                                            Text(
                                              _isSelectionMode ? "Select Records" : "Submission History",
                                            ...
                                            Text(
                                              ...
                                              style: TextStyle(
                                                  fontSize: 12, 
                                                  color: _isSelectionMode ? const Color(0xFF00897B) : Colors.grey.shade600, 
                                        ...
                                        child: Row(
                                          children: [
                                            _HoverZoomLink(
                                              ...
                                              child: Text(
                                                _selectedIds.length == _complaints.length ? "DESELECT ALL" : "SELECT ALL",
                                                style: const TextStyle(color: Colors.green, fontWeight: FontWeight.w900, fontSize: 13, letterSpacing: 0.5),
                                              ),
                                            ),
                                            const Spacer(),
                                            _HoverZoomLink(
                                              onTap: _cancelSelection,
                                              child: const Text("CANCEL", style: TextStyle(color: Colors.grey, fontWeight: FontWeight.w900, fontSize: 13, letterSpacing: 0.5)),
                                            ),
                                            if (_selectedIds.isNotEmpty) ...[
                                              const SizedBox(width: 24),
                                              _HoverZoomLink(
                                                onTap: _handleClearAll,
                                                child: const Text("DELETE", style: TextStyle(color: Colors.red, fontWeight: FontWeight.w900, fontSize: 13, letterSpacing: 0.5)),
                                              ),
                                            ],
                                          ],
                                        ),
  ...
  Widget _buildDismissibleItem(dynamic complaint, int index, Animation<double> animation, {bool isRemoving = false}) {
    ...
    final Widget card = ComplaintItemCard(
      ...
      isSelected: _selectedIds.contains(cid),
      onDelete: () {
        if (_isSelectionMode) {
          _toggleSelection(cid);
        } else {
          int curIdx = _complaints.indexWhere((c) => (c['complaint_id'] ?? c['id']).toString() == cid);
          if (curIdx != -1) _handleDelete(curIdx);
        }
      },
      onDoubleTap: () => _enterSelectionMode(cid),
      onLongPress: () {
        if (!Responsive.isDesktop(context)) {
          _enterSelectionMode(cid);
        }
      },
      onToggleSelection: () => _toggleSelection(cid),
    );
    ...
    return Padding(
      ...
      child: Dismissible(
        ...
        direction: _isSelectionMode ? DismissDirection.none : DismissDirection.horizontal, // Disable swipe when in selection mode
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: widget.isSelected 
                ...
                : (widget.isHighlighted ? const Color(0xFFF1F8E9) : Colors.white),
            borderRadius: BorderRadius.circular(24),
            border: Border.all(
                color: widget.isSelected
                ...
                width: 2),
            boxShadow: widget.isSelected || widget.isHighlighted || _isHovered
                ? [
                    BoxShadow(
                        color: (widget.isSelected || widget.isHighlighted ? const Color(0xFF00897B) : Colors.black).withOpacity(0.1),
                        blurRadius: 20,
                        offset: const Offset(0, 10))
                  ]
                ...
                children: [
                  Row(
                    ...
                    children: [
                      Expanded(
                        child: Padding(
                          padding: EdgeInsets.only(right: widget.isSelectionMode ? 50 : 0),
                      ...
                      Padding(
                        padding: EdgeInsets.only(right: widget.isSelectionMode ? 30 : 0),
                        child: Container(
                  ...
                  Row(
                    children: [
                      ...
                      const Spacer(),
                      if (!isDesktop && !widget.isSelectionMode)
