...
class _AnalyticsScreenState extends State<AnalyticsScreen> with TickerProviderStateMixin {
            ...
            children: [
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: Icon(Icons.download_rounded, color: AppColors.tealText, size: iconSize),
                ),
              ),
            ],
          ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: isMobile ? 230 : 200, child: chart), 
          if (legend != null) ...[
  ...
  void _showAllEtasModal(BuildContext context) {
    ...
    Widget modalContent(StateSetter setModalState) {
      ...
      return Container(
        padding: EdgeInsets.fromLTRB(28, isMobile ? 12 : 24, 28, 0),
        decoration: BoxDecoration(
          color: Colors.white,
  ...
  Widget _buildTruckDonutChart() {
    ...
    final double radius = isMobile ? 40 : 25;
    final double centerSpaceRadius = isMobile ? 65 : 40;

    return PieChart(PieChartData(sections: [
      if (active > 0) PieChartSectionData(value: active, color: Colors.green, radius: radius, title: '${active.toInt()}', titleStyle: TextStyle(fontSize: isMobile ? 14 : 11, fontWeight: FontWeight.w900, color: Colors.white), titlePositionPercentageOffset: 0.5),
      if (full > 0) PieChartSectionData(value: full, color: Colors.amber, radius: radius, title: '${full.toInt()}', titleStyle: TextStyle(fontSize: isMobile ? 14 : 11, fontWeight: FontWeight.w900, color: Colors.white), titlePositionPercentageOffset: 0.5),
      if (idle > 0) PieChartSectionData(value: idle, color: Colors.grey.shade300, radius: radius, title: '${idle.toInt()}', titleStyle: TextStyle(fontSize: isMobile ? 14 : 11, fontWeight: FontWeight.w900, color: Colors.black54), titlePositionPercentageOffset: 0.5),
    ...
    return PieChart(PieChartData(sections: [
      if (pending > 0)
        PieChartSectionData(
          ...
          title: '${pending.toInt()}', titleStyle: TextStyle(fontSize: isMobile ? 14 : 11, fontWeight: FontWeight.w900, color: Colors.white),
          titlePositionPercentageOffset: 0.5,
        ),
      if (inProgress > 0)
        PieChartSectionData(
          value: inProgress, color: Colors.blue, radius: radius,
          title: '${inProgress.toInt()}', titleStyle: TextStyle(fontSize: isMobile ? 14 : 11, fontWeight: FontWeight.w900, color: Colors.white),
          titlePositionPercentageOffset: 0.5,
        ),
      if (resolved > 0)
        PieChartSectionData(
          value: resolved, color: Colors.green, radius: radius,
          title: '${resolved.toInt()}', titleStyle: TextStyle(fontSize: isMobile ? 14 : 11, fontWeight: FontWeight.w900, color: Colors.white),
          titlePositionPercentageOffset: 0.5,
        ),
    ], centerSpaceRadius: centerSpaceRadius, sectionsSpace: 2));
  }
...
class _FullDetailsModalState extends State<_FullDetailsModal> {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Container(
      padding: EdgeInsets.fromLTRB(28, widget.isMobile ? 12 : 24, 28, 0), 
      decoration: BoxDecoration(
        color: Colors.white, 
        borderRadius: widget.isMobile ? const BorderRadius.vertical(top: Radius.circular(32)) : BorderRadius.circular(32)
      ), 
      constraints: BoxConstraints(
        maxHeight: widget.isMobile ? MediaQuery.of(context).size.height * 0.65 : 550, // Balanced height
      ),
      child: isModalLoading 
        ? Column(
            children: [
              if (widget.isMobile) 
                Container(width: 40, height: 4, margin: const EdgeInsets.only(top: 12), decoration: BoxDecoration(color: Colors.grey.shade300, borderRadius: BorderRadius.circular(10))),
              const Spacer(),
              ...
              Flexible(
                child: SingleChildScrollView(
                  ...
                  child: Container(
                    ...
                    child: Text(
                      _geminiSummaryLocal ?? "AI insights temporarily unavailable.", 
                      textAlign: _geminiSummaryLocal == null ? TextAlign.center : TextAlign.left,
